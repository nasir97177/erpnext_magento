...
def get_magento_settings():
	...
	if d.magento_url:
		if d.api_access_token:
			...
			frappe.throw(_("Magento API access token isn't configured in Magento settings"), MagentoError)
	else:
		frappe.throw(_("Magento store URL isn't configured in Magento settings"), MagentoError)
...
def value_search_str(field, value, condition=None):
	field_str = f"searchCriteria[filter_groups][0][filters][0][field]={field}"
	value_str = f"&searchCriteria[filter_groups][0][filters][0][value]={value}"
	if condition:
		condition_str = "&searchCriteria[filter_groups][0][filters][0][condition_type]={condition}"
	else:
		condition_str = ""
	return f"{field_str}{value_str}{condition_str}"
...
def get_filtering_condition():
	...
	if magento_settings.last_sync_datetime:
		...
		filter = f"searchCriteria[filter_groups][0][filters][0][field]=updated_at\
&searchCriteria[filter_groups][0][filters][0][value]={utc_dt.strftime('%Y-%m-%d %H:%M:%S')}\
...
def get_magento_country_id_by_name(country_name):
	for country in get_request('directory/countries'):
		if country.get("full_name_locale") == country_name or country.get("full_name_english") == country_name:
...
def get_magento_item_attribute_details_by_name(item_attribute_name):
	for magento_item_attribute in get_request("products/attributes?searchCriteria[pageSize]=250")["items"]:
